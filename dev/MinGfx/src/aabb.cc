...
AABB::AABB(const Mesh &mesh, unsigned int tri_id) {
    std::vector<unsigned int> indices = mesh.triangle_verticesReadOnly(tri_id);
    Point3 a = mesh.vertexReadOnly(indices[0]);
    Point3 b = mesh.vertexReadOnly(indices[1]);
    Point3 c = mesh.vertexReadOnly(indices[2]);
...
AABB::AABB(const Mesh &mesh) {
    ...
    for (int i=0; i < mesh.num_vertices(); i++) {
        Point3 a = mesh.vertexReadOnly(i);
