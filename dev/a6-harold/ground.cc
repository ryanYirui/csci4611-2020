                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    // 2 points
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d[0], &start);  //first point
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d.back(), &end);  //last point
    Vector3 Plane_pass;
    Vector3 Plane_normal;
    Plane_pass = (start - end).ToUnit();
    Plane_normal = -Plane_pass;
    Plane_normal = Plane_normal.Cross(Vector3(0, 1, 0)).ToUnit();   //set the normal 
    ...
    // defined in step 1.
    std::vector<Point3> convert;
    int temp1 = stroke2d.size();
    float t;
    Point3 pt;
    for (int j = 0; j < temp1; j++) {
        Point3 In_3d = GfxMath::ScreenToNearPlane(view_matrix, proj_matrix, stroke2d[j]);
        Ray eye3d(eye, (In_3d - eye).ToUnit());
        eye3d.IntersectPlane(start, Plane_normal, &t, &pt);
        convert.push_back(pt);
    }
    ...
    int n = ground_mesh_.num_vertices();
    for (int i=0; i<n; i++) {
        Point3 P = ground_mesh_.vertexReadOnly(i); // original vertex

        // adjust P according to equations...
        
        float h = hfunc(Plane_normal, convert, P.ClosestPointOnPlane(start, Plane_normal));
        // follow the function that given in the handout
        // there are 2 cases that need to be considered one is h !=0  if h = 0 then we simply do not need to modfied anything
        float dis = P.DistanceToPlane(start, Plane_normal);
        float w = 0.0;
        float tem = 1 - pow(dis / 5, 2);
